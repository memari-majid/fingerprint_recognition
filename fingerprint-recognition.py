...
path = '/fingerprint/fingerprint_recognition/dataset/'
x_real = np.load(f'{path}/x_real.npz')['data']
y_real = np.load(f'{path}/y_real.npy')
x_easy = np.load(f'{path}/x_easy.npz')['data']
y_easy = np.load(f'{path}/y_easy.npy')
x_medium = np.load(f'{path}x_medium.npz')['data']
y_medium = np.load(f'{path}/y_medium.npy')
x_hard = np.load(f'{path}/x_hard.npz')['data']
y_hard = np.load(f'{path}/y_hard.npy')
...
train_gen = DataGenerator(x_train, label_train, x_real,
                          label_real_dict, shuffle=True)
val_gen = DataGenerator(x_val, label_val, x_real,
                        label_real_dict, shuffle=False)
...
random_img = seq.augment_image(random_img).reshape(
    (1, 90, 90, 1)).astype(np.float32) / 255.

# matched image
match_key = random_label.astype(str)
match_key = ''.join(match_key).zfill(6)

rx = x_real[label_real_dict[match_key]].reshape(
    (1, 90, 90, 1)).astype(np.float32) / 255.
